...
@section('title', 'Edit Admit Card - Sarkari Result Admin')
...
@section('content')
    <div class="card">
        <div class="card-header">
            <h3 class="card-title">Edit Admit Card Information</h3>
        </div>
        
        <form id="editAdmitCardForm" action="{{ route('admin.admit-cards.update', $admitCard) }}" method="POST">
            @csrf
            @method('PUT')
            <div class="card-body">
                <div class="row">
                        ...
                        <div class="form-group">
                            ...
                            <select class="form-control @error('job_post_id') is-invalid @enderror" id="job_post_id" name="job_post_id" required>
                                ...
                                @foreach($jobPosts as $jobPost)
                                    <option value="{{ $jobPost->id }}" {{ old('job_post_id', $admitCard->job_post_id) == $jobPost->id ? 'selected' : '' }}>
                                        {{ $jobPost->title }}
                                    </option>
                                @endforeach
                            </select>
                    ...
                    <div class="col-md-6">
                        <div class="form-group">
                            ...
                            <select class="form-control @error('status') is-invalid @enderror" id="status" name="status" required>
                                <option value="active" {{ old('status', $admitCard->status) === 'active' ? 'selected' : '' }}>Active</option>
                                <option value="inactive" {{ old('status', $admitCard->status) === 'inactive' ? 'selected' : '' }}>Inactive</option>
                            </select>
                            @error('status')
                                <span class="invalid-feedback">{{ $message }}</span>
                            @enderror
                ...
                <div class="form-group">
                    ...
                    <input type="text" class="form-control @error('title') is-invalid @enderror" id="title" name="title" 
                           value="{{ old('title', $admitCard->title) }}" required>
                    @error('title')
                        <span class="invalid-feedback">{{ $message }}</span>
                    @enderror
                </div>
                ...
                <div class="form-group">
                    ...
                    <input type="text" class="form-control @error('organization') is-invalid @enderror" id="organization" name="organization" 
                           value="{{ old('organization', $admitCard->organization) }}" required>
                    @error('organization')
                        <span class="invalid-feedback">{{ $message }}</span>
                    @enderror
                </div>
                ...
                <div class="form-group">
                    ...
                    <textarea class="form-control @error('description') is-invalid @enderror" id="description" name="description" 
                              rows="4">{{ old('description', $admitCard->description) }}</textarea>
                    @error('description')
                        <span class="invalid-feedback">{{ $message }}</span>
                    @enderror
                </div>
                    ...
                    <div class="col-md-6">
                        <div class="form-group">
                            ...
                            <input type="date" class="form-control @error('exam_date') is-invalid @enderror" id="exam_date" name="exam_date" 
                                   value="{{ old('exam_date', $admitCard->exam_date?->format('Y-m-d')) }}">
                            @error('exam_date')
                                <span class="invalid-feedback">{{ $message }}</span>
                            @enderror
                        </div>
                    ...
                    <div class="col-md-6">
                        <div class="form-group">
                            ...
                            <input type="url" class="form-control @error('download_link') is-invalid @enderror" id="download_link" name="download_link" 
                                   value="{{ old('download_link', $admitCard->download_link) }}" required>
                            @error('download_link')
                                <span class="invalid-feedback">{{ $message }}</span>
                            @enderror
                        </div>
                ...
                <div class="form-group">
                    ...
                    <input type="url" class="form-control @error('official_website') is-invalid @enderror" id="official_website" name="official_website" 
                           value="{{ old('official_website', $admitCard->official_website) }}">
                ...
                <div class="form-group">
                    ...
                    <textarea class="form-control @error('instructions') is-invalid @enderror" id="instructions" name="instructions" 
                              rows="4">{{ old('instructions', $admitCard->instructions) }}</textarea>
            ...
            <div class="card-footer">
                <button type="submit" class="btn btn-primary" id="submitBtn">
                    <i class="fas fa-save"></i> Update Admit Card
                </button>
                <a href="{{ route('admin.admit-cards.index') }}" class="btn btn-secondary">
...
@section('js')
    ...
    <script>
        $(document).ready(function() {
            ...
            $('#editAdmitCardForm').on('submit', function(e) {
                e.preventDefault();
                ...
                $.ajax({
                    ...
                    success: function(response) {
                        toastr.success('Admit card updated successfully!');
                        setTimeout(function() {
                    ...
                    error: function(xhr) {
                        if (xhr.status === 422) {
                            ...
                            toastr.error('An error occurred while updating the admit card.');
                        }
                    },
                    complete: function() {
